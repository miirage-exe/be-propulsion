...
plage_opr = np.linspace(2, 60, 40) # OPR de 20 à 60
plage_tt = [1400, 1600, 1800, 2000, 2200] # Différentes TtSortieCC en Kelvin
...
for tt_current in plage_tt:
    rendements = []
    for opr_current in plage_opr:
        # OPRfan = opr_current
        lmbda = 11
        eta = calculer_rendement(40, 1600, stateInit)
        rendements.append(eta)
    
    plt.plot(plage_opr, rendements, label=f'Tt4 = {tt_current}')

plt.title(f"Rendement Thermique en fonction du taux de compression global pour plusieurs Tt4")
